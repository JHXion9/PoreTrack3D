...
if __name__ == '__main__':  # 确保在 if __name__ == '__main__': 块中运行多进程代码
    ...
    # 单独跑第一帧，生成后续帧需要的pr_new(psift需要)
    process_time_step('First-Frame', 0, 1, completed_steps, lock)
    remaining_times = times - 1

    steps_per_process = math.ceil(remaining_times / num_processes)  # 向上取整，确保所有步骤都被执行
    process_list = []
    ...
    for frame in range(1
        ...
        while loop:
            loop = get_ply(project_path, str(human_number), frame)
            if Loop_max >20:
