...
%读入参数
file_content = fileread("/media/Trajectory3D/tools/ply/get_patch_parameters.txt");
...
for i = 1:length(lines)
    ...
    if contains(line, '=')
        ...
        switch key
            case 'patch_size'
                py_patch_size = str2double(value);
            case 'patch_folder_path'
                folder_path = value;
            case 'patch_folder_path_pathkp'
                folder_path_pathkp = value;
            case 'patch_save_path'
                save_path = value;
            otherwise
                warning('未知参数: %s', key);
        end
...
% 输出参数
fprintf('py_patch_size: %s\n', py_patch_size);
fprintf('folder_path: %s\n', folder_path);
fprintf('folder_path_pathkp: %s\n', folder_path_pathkp);
fprintf('save_path: %s\n', save_path);
...
%save_path ='E:\make_dataset\human372_64patch\descriptors'

% 获取文件夹中所有文件的信息
files = dir(fullfile(folder_path, '*.png'));  % 假设文件夹中包含 .jpg 文件
...
for i = 1:total_files
    ...
    % 提取补丁
    patch = extract_patches(image, kp, py_patch_size/2-1);
