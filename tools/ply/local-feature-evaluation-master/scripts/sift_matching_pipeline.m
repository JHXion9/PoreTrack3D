...
for i = 1:length(DATASET_NAMES)
    ...
    DATASET_PATH = ['E:\make_dataset\' DATASET_NAMES{i}];

    % TODO: Change this to where VLFeat is located.
    VLFEAT_PATH = 'E:\make_dataset\vlfeat-0.9.21';
    ...
    siftDESCRIPTOR_PATH = fullfile(DATASET_PATH, 'siftdescriptors');
    ...
    if ~exist(siftDESCRIPTOR_PATH, 'dir')
        mkdir(siftDESCRIPTOR_PATH);
    end
    ...
    siftdescriptor_paths= cell(num_images, 1);
    ...
    for i = 3:length(image_files)
        ...
        siftdescriptor_paths{i-2} = fullfile(siftDESCRIPTOR_PATH, [image_name '.bin']);
    end
